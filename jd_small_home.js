...
!(async () => {
  ...
  $.inviteCodes = await getAuthorShareCode('https://raw.githubusercontent.com/Aaron-lv/linmudayeTeam/master/shareCodes/jd_linmudayeSmallHomeInviteCode.json')
  if (!$.inviteCodes) {
    $.http.get({url: 'https://purge.jsdelivr.net/gh/Aaron-lv/linmudayeTeam@master/shareCodes/jd_linmudayeSmallHomeInviteCode.json'}).then((resp) => {}).catch((e) => $.log('刷新CDN异常', e));
    await $.wait(1000)
    $.inviteCodes = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/Aaron-lv/linmudayeTeam@master/shareCodes/jd_linmudayeSmallHomeInviteCode.json')
  }
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
...
async function helpFriends() {
  // await linmudayeInviteCode();
  // if (!$.inviteCodes) await linmudayeInviteCodeCDN();
