...
!(async () => {
  ...
  await updateInviteCodeCDN('https://cdn.jsdelivr.net/gh/linmudaye/updateTeam@main/jd_updateSmallHomeInviteCode.json');
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      $.token = $.helpToken[i];
    ...
    $.get({url,headers:{
      ...
      try {
        if (err) {
          ...
          $.get({url: `https://raw.githubusercontent.com/linmudaye/updateTeam/main/shareCodes/jd_updateSmallHomeInviteCode.json`, 'timeout': 10000},(err, resp, data)=>{
