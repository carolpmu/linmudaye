...
!(async () => {
  ...
  await updateInviteCodeCDN('https://cdn.jsdelivr.net/gh/linmudaye/updateTeam@master/jd_updateSmallHomeInviteCode.json');
  for (let i = 0; i < cookiesArr.length; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      $.token = $.helpToken[i];
...
function updateInviteCode(url = 'https://raw.githubusercontent.com/linmudaye/updateTeam/master/shareCodes/jd_updateSmallHomeInviteCode.json') {
  return new Promise(resolve => {
    $.get({url,headers:{
      ...
      try {
        if (err) {
          ...
          $.get({url: `https://raw.githubusercontent.com/linmudaye/updateTeam/master/shareCodes/jd_updateSmallHomeInviteCode.json`, 'timeout': 10000},(err, resp, data)=>{
