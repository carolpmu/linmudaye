...
!(async () => {
    ...
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/linmudaye/updateTeam/master/shareCodes/qjd.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://gitee.com/linmudaye/updateTeam/raw/master/qjd.json');}catch (e) {}
        if(!res){res = [];}
    }
    if(res.length > 0){
        let len = 3;
