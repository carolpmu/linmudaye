...
!(async () => {
    ...
    await $.wait(10000);
    let res = [];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/linmudaye/updateTeam/master/city.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://gitee.com/linmudaye/updateTeam/raw/master/city.json');}catch (e) {}
        if(!res){res = [];}
    }
    if(res.length > 0){
        if(res.length > 3){
