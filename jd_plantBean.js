...
async function jdPlantBean() {
  try {
    ...
    if ($.plantBeanIndexResult && $.plantBeanIndexResult.code === '0' && $.plantBeanIndexResult.data) {
      ...
      // 报告运行次数
      $.get({
        url: `https://cdn.linmudaye.lu/api/runTimes?activityId=bean&sharecode=${$.myPlantUuid}`,
        headers: {
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `https://cdn.linmudaye.lu/api/bean/${randomCount}`, headers:{'Host':'api.sharecode.ga'}, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
